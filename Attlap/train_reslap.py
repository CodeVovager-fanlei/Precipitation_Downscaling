...
for i in range(5):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss <= best_val_loss:
            ...
            epochs_no_improve = 0
            torch.save(model.state_dict(), './model_parameter/reslap_CBAM_1e-4_best.pth')  # Save the best model
        else:
            epochs_no_improve += 1
    ...
    if not early_stop:
        torch.save(model.state_dict(), './model_parameter/reslap_CABAM_1e-4.pth')
        print('Final model saved to rrdb_model.pth')
    else:
        print('Best model saved to rrdb_model_best.pth')
