...
class ChannelAttention(nn.Module):
    def __init__(self, channel, reduction=16):
        super().__init__()     #继承nn.module的构造函数
        #写有那些层，就是初始化方法
    ...
    #自定义一个forword方法，不用自定义反向传播，只用写前向传播
    def forward(self, x):
...
class ResidualDenseBlock(nn.Module):
    def __init__(self, num_feat=64, num_grow_ch=32):
        ...
        # self.alpha = nn.Parameter(torch.tensor(0.2))  # 可训练的残差系数
...
# 修改后的 ResLap 网络
class RRDBLap(nn.Module):
    def __init__(self, num_feat=64):
        super(RRDBLap, self).__init__()
        ...
        self.dconv = nn.ModuleList([
            ...
            nn.Upsample(size=(100, 159), mode='bicubic', align_corners=False)
            # EUCB(in_channels=32, out_channels=32, size=(100, 159))
        ])
        ...
        self.res_construction = nn.ModuleList([
            ...
            nn.Upsample(size=(100, 159), mode='bicubic', align_corners=False)
            # EUCB(in_channels=3, out_channels=3, size=(100, 159))
        ])

        # 初始化权重
        default_init_weights([self.conv1, *self.conv2])
        #     特征融合
        self.conv_up = nn.Sequential(nn.ConvTranspose2d(32, 3, kernel_size=5, stride=3, padding=1))
        self.fusion1 = CGAFusion(3)
        self.fusion2 = CGAFusion(3)
        self.fusion3 = CGAFusion(3)
        self.up1 = nn.ConvTranspose2d(3, 3, kernel_size=4, stride=2, padding=1)
        self.up2 = nn.Upsample(size=(100, 159), mode='bicubic', align_corners=False)
    ...
    def forward(self, x):
        ...
        x2 = self.dconv[1](x2)
        r2 = self.conv2[1](x2)
        r2 = self.fusion1(F1, r2)
        F2 = self.up2(r2)  # 融合之后再上采样
...
if __name__ == "__main__":
    ...
    a = torch.rand((128, 15, 6, 9)).to(device)
    r = model(a)
