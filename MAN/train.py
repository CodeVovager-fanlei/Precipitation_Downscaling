...
model = MANModel(input_shape, output_shape).to(device)
for i in range(5):
    print(f'第{i}折')
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss <= best_val_loss:
            ...
            epochs_no_improve = 0
            torch.save(model.state_dict(), './model_parameter/MAN_Model_best.pth')  # Save the best model
        else:
            epochs_no_improve += 1
    ...
    if not early_stop:
        torch.save(model.state_dict(), './model_parameter/MAN_Model.pth')
        print('Final model saved to man_model.pth')
    else:
        print('Best model saved to man_model_best.pth')
